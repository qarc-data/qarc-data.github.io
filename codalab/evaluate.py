...
class ClassificationEvaluator:
    ...
    def evaluate(self, y_true, y_pred):
        ...
        results = {}
        results["micro_accuracy"] = int(100 * micro_accuracy) / 100
        ...
        results["macro_accuracy"] = int(100 * macro_accuracy) / 100
...
class CombinedEvaluator:
    ...
    def extract_follow_ups(self, y_true, y_pred):
        extracted = [(y_t, y_p) for (y_t, y_p) in zip(y_true, y_pred) if y_t.lower() not in self.labels]
        if extracted:
...
def evaluate(gold_file, prediction_file, mode='follow_ups'):
    ...
    assert all(gt['utterance_id'] == pred['utterance_id'] for gt, pred in zip(ground_truths, predictions)), \
        "Utterance ID misalignment detected"
    ...
    ground_truths = [prepro(sample['answer']) for sample in ground_truths]
    predictions = [prepro(sample['answer']) for sample in predictions]
